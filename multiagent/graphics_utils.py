...
def move_to(current_object, x, y=None,
            d_o_e=lambda arg: _root_window.dooneevent(arg),
            d_w=tkinter._tkinter.DONT_WAIT):
    if y is None:
        try:
    ...
    horiz = True
    new_coords = []
    current_x, current_y = _canvas.coords(current_object)[0:2]  # first point
    for coord in _canvas.coords(current_object):
        if horiz:
            inc = x - current_x
        else:
            inc = y - current_y
    ...
    _canvas.coords(current_object, *new_coords)
    d_o_e(d_w)


def move_by(current_object, x, y=None,
            d_o_e=lambda arg: _root_window.dooneevent(arg),
    ...
    d_o_e(d_w)
    if lift:
        _canvas.tag_raise(current_object)


def write_postscript(filename):
    """Writes the current canvas to a postscript file."""
    with open(filename, 'w') as ps_file:
