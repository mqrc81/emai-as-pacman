...
class Grades:
    ...
    def add_pre_req(self, question, pre_req):
        self.pre_reqs[question].add(pre_req)
    ...
    def grade(self, grading_module, exception_map={}, bonus_pic=False):
        ...
        for q in self.questions:
            ...
            incompleted = self.pre_reqs[q].difference(completed_questions)
...
""")
    ...
    def add_error_hints(self, exception_map, error_instance, question_num):
        ...
        # don't include the HTML if we have no error hint
        if not error_hint:
            return ''
    ...
    def produce_grade_scope_output(self):
        ...
        for name in self.questions:
            test_out = {'name': name, 'score': self.points[name], 'max_score': self.maxes[name]}
            # test name
            # test score
            # others
        ...
        # file output
        with open('gradescope_response.json', 'w') as out_file:
            json.dump(out_dct, out_file)
        return
